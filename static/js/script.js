...
document.addEventListener('DOMContentLoaded', function() {
            ...
            <div class="search-result-item" onclick="goToProduct(${product.id})" data-product-id="${product.id}">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <div class="text-muted small">Rp ${parseFloat(product.price).toLocaleString('id-ID')}</div>
                    </div>
                </div>
            </div>
        `).join('');
    ...
    function updateCartCount() {
        ...
        if (cartBadge) {
            // You can implement AJAX call to get actual cart count
    ...
    function disconnectFloatingChatSocket() {
        if (floatingChatSocket) {
            floatingChatSocket.disconnect();
            floatingChatSocket = null;
        }
    }
                ...
                if (data.success) {
                    ...
                    if (data.messages && data.messages.length > 0) {
                        data.messages.forEach(message => {
                            addFloatingMessage(message.message, message.sender_type, message.timestamp, message.product_id);
                        });
                        scrollFloatingChatToBottom();

                        // Update unread count
                        const adminMessages = data.messages.filter(msg => msg.sender_type === 'admin');
