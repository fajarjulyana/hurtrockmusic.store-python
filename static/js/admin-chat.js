...
class AdminChat {
    ...
    displayChatRooms(rooms) {
        ...
        if (rooms.length === 0) {
            roomsList.innerHTML = `
                <div class="text-center p-4 text-muted">
                    ...
                    <small>Chat akan muncul di sini ketika buyer mengirim pesan</small>
                </div>
            `;
            return;
        }
        ...
        roomsList.innerHTML = rooms.map(room => {
            ...
            const unreadIndicator = room.unread_count > 0 ? '<i class="fas fa-circle text-primary" style="font-size: 8px; margin-left: 5px;"></i>' : '';
            
            // Truncate message for display
            const messagePreview = lastMessage ? 
                (lastMessage.content.length > 35 ? lastMessage.content.substring(0, 35) + '...' : lastMessage.content) : 
                'Belum ada pesan';
                ...
                <div class="chat-room-item ${room.unread_count > 0 ? 'unread' : ''}"
                    ...
                    <div class="room-header">
                        <div class="customer-name">
                            ${this.escapeHtml(room.buyer_name)}
                            ${unreadIndicator}
                        </div>
                        ${unreadBadge}
                    </div>
                    <div class="customer-email">${this.escapeHtml(room.buyer_email)}</div>
                    <div class="last-message">${this.escapeHtml(messagePreview)}</div>
                    <div class="message-time">
                        ${timeAgo}
                        ${room.unread_count > 0 ? '<i class="fas fa-envelope text-primary ms-1" style="font-size: 10px;"></i>' : ''}
                    </div>
                </div>
            `;
        }).join('');
    ...
    selectBuyerRoom(roomName, buyerName, buyerEmail, buyerId) {
        ...
        if (selectedRoomElement) {
            ...
            const unreadBadge = selectedRoomElement.querySelector('.unread-count');
            if (unreadBadge) {
                unreadBadge.remove();
            }
            const unreadIcons = selectedRoomElement.querySelectorAll('.fa-circle, .fa-envelope');
            unreadIcons.forEach(icon => icon.remove());
        }
    }
        ...
        if (searchInput) {
            ...
            if (searchIcon) {
                searchIcon.addEventListener('click', () => {
                    this.loadChatRooms(searchInput.value.trim());
                });
            }
