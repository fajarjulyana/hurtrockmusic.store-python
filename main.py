...
def initialize_database():
    ...
    try:
        ...
        # Setup Django chat service - temporarily disabled for initial setup
        print("[INFO] Django chat service setup temporarily disabled")
...
def print_professional_label(order_id):
    """
    Generate professional shipping label with barcode and QR code
    Similar to modern courier service labels
    """
    order = models.Order.query.get_or_404(order_id)
    store_profile = models.StoreProfile.get_active_profile()
    ...
    # A4 landscape size for professional printing
    width, height = 842, 595  # A4 landscape in points (297x210mm)
    p = canvas.Canvas(buffer, pagesize=(width, height))

    # Color scheme
    primary_color = colors.HexColor('#FF6B35')  # Hurtrock orange
    accent_color = colors.HexColor('#10B981')   # Success green
    dark_color = colors.HexColor('#1F2937')    # Dark gray
    light_bg = colors.HexColor('#F9FAFB')      # Light background
    ...
    brand_name = "Hurtrock"
    p.drawString(50, height - 65, brand_name)
    
    # Generate and place barcode
    order_number = f"{order.id:08d}"
    tracking_code = order.tracking_number or f"HRT{order_number}"
    
    try:
        # Generate barcode using reportlab
        from reportlab.graphics.barcode import code128
        barcode_obj = code128.Code128(tracking_code, 
                                      barWidth=1.8, 
                                      barHeight=40,
                                      humanReadable=False)
        
        # Position barcode in header
        barcode_obj.drawOn(p, width - 350, height - 80)
        
        # Add barcode text
        p.setFont("Helvetica-Bold", 14)
        p.drawCentredText(width - 225, height - 90, f"NO PESANAN {order_number}")
        ...
        for i in range(30):
            x = width - 300 + (i * 6)
            bar_height = 35 if (i + ord(tracking_code[i % len(tracking_code)])) % 3 == 0 else 25
            p.rect(x, height - 75, 3, bar_height, fill=1, stroke=0)
    ...
    # Main content area
    content_y = height - 130
    
    # Left section - Items
    p.setFillColor(dark_color)
    p.setFont("Helvetica-Bold", 20)
    p.drawString(50, content_y, "Jenis Barang")
    ...
    for item in order.order_items:
        if item_count < 4:  # Show max 4 items
            item_name = item.product.name
            if len(item_name) > 35:
                item_name = item_name[:32] + "..."
            p.drawString(70, content_y, f"• {item_name}")
            p.drawString(70, content_y - 15, f"  {item.quantity} pcs")
            content_y -= 35
            item_count += 1
    
    if len(order.order_items) > 4:
        p.drawString(70, content_y, f"• +{len(order.order_items) - 4} item lainnya")
        content_y -= 25
    
    # Separator line
    p.setStrokeColor(dark_color)
    p.setLineWidth(2)
    p.line(50, content_y - 10, 350, content_y - 10)
    
    # Price section
    content_y -= 40
    p.setFillColor(dark_color)
    p.setFont("Helvetica-Bold", 20)
    p.drawString(50, content_y, "Harga")
    
    content_y -= 35
    p.setFont("Helvetica-Bold", 24)
    p.drawString(50, content_y, order.formatted_total)
    
    # Weight info
    total_weight = sum(item.quantity * (item.product.weight or 100) for item in order.order_items) / 1000
    content_y -= 45
    p.setFont("Helvetica-Bold", 16)
    p.drawString(50, content_y, f"Berat {total_weight:.1f}kg")
    
    # Center section - Recipient
    recipient_x = 400
    p.setFillColor(dark_color)
    p.setFont("Helvetica-Bold", 24)
    p.drawString(recipient_x, height - 165, "Penerima")
    
    recipient_y = height - 200
    p.setFont("Helvetica-Bold", 18)
    p.drawString(recipient_x, recipient_y, order.user.name)
    ...
    if order.user.address:
        ...
        for line in lines[:4]:  # Max 4 lines
            p.drawString(recipient_x, recipient_y, line)
            recipient_y -= 18
    ...
    if order.user.phone:
        ...
        p.drawString(recipient_x, recipient_y, f"Telp: {order.user.phone}")
    
    # Right section - Sender
    sender_x = width - 220
    p.setFillColor(dark_color)
    p.setFont("Helvetica-Bold", 24)
    p.drawString(sender_x, height - 165, "Pengirim")
    
    sender_y = height - 200
    p.setFont("Helvetica-Bold", 16)
    sender_name = store_profile.store_name if store_profile else "Hurtrock Music"
    p.drawString(sender_x, sender_y, sender_name)
    
    # Sender address
    sender_y -= 22
    p.setFont("Helvetica", 11)
    if store_profile:
        sender_address = store_profile.formatted_address
        # Split address for sender
        address_parts = sender_address.split(', ')
        for part in address_parts[:4]:
            if len(part) > 25:
                part = part[:22] + "..."
            p.drawString(sender_x, sender_y, part)
            sender_y -= 14
        
        if store_profile.store_phone:
            sender_y -= 8
            p.setFont("Helvetica-Bold", 10)
            p.drawString(sender_x, sender_y, f"Telp: {store_profile.store_phone}")
    ...
    try:
        ...
        # QR code data
        qr_data = f"""Hurtrock Music Store
Order: {order.id:08d}
...
www.hurtrock.com"""
        ...
        # Draw QR code
        p.drawInlineImage(qr_buffer, 50, footer_y - 70, width=70, height=70)
        ...
        p.setStrokeColor(dark_color)
        p.rect(50, footer_y - 70, 70, 70, fill=1, stroke=1)
        p.setFillColor(dark_color)
        p.setFont("Helvetica-Bold", 8)
        p.drawCentredText(85, footer_y - 35, "QR CODE")
    
    # Service information
    info_x = 140
    p.setFillColor(dark_color)
    p.setFont("Helvetica-Bold", 12)
    p.drawString(info_x, footer_y - 15, f"Layanan Kirim: {order.courier_service or 'Reguler'}")
    
    p.setFont("Helvetica", 11)
    p.drawString(info_x, footer_y - 30, f"Tanggal Order: {order.created_at.strftime('%d/%m/%Y %H:%M')}")
    p.drawString(info_x, footer_y - 45, f"Estimasi Tiba: {order.estimated_delivery_days or 3} hari kerja")
    p.drawString(info_x, footer_y - 60, f"Berat Total: {total_weight:.1f} kg")
    
    # Website and contact
    p.setFont("Helvetica-Bold", 10)
    website = store_profile.store_website if store_profile else "www.hurtrock.com"
    p.drawString(info_x, footer_y - 80, website)
    
    # Green accent corner
    p.setFillColor(accent_color)
    triangle_points = [(width - 120, height), (width, height), (width, height - 120)]
    p.polygon(triangle_points, fill=1, stroke=0)
    
    # Status badge
    p.setFillColor(colors.white)
    p.setFont("Helvetica-Bold", 12)
    status_text = order.status.upper().replace('_', ' ')
    p.drawString(width - 110, height - 30, status_text)
    ...
    p.drawCentredText(width/2, 10, "Terima kasih telah berbelanja di Hurtrock Music Store")
    ...
    return send_file(
        buffer,
        as_attachment=True,
        download_name=f'professional_shipping_label_{order.id}.pdf',
...
def print_order_label(order_id):
    ...
    # Create professional shipping label with barcode
    buffer = io.BytesIO()
    
    # Professional label dimensions (A5 landscape = 595x420 points)
    width, height = 595, 420
    p = canvas.Canvas(buffer, pagesize=(width, height))

    # Colors
    brand_color = colors.HexColor('#FF6B35')  # Orange brand color
    accent_color = colors.HexColor('#34D399')  # Green accent
    ...
    store_name = store_profile.store_name if store_profile else "Hurtrock"
    p.drawString(left_margin, height - 55, store_name)
    
    # Order number barcode section
    order_number = f"{order.id:08d}"
    tracking_text = order.tracking_number or f"HRT{order_number}"
    ...
    try:
        ...
        barcode_obj = code128(tracking_text, writer=ImageWriter())
        
        # Save barcode to temporary buffer
        barcode_buffer = io.BytesIO()
        barcode_obj.write(barcode_buffer)
        barcode_buffer.seek(0)
        
        # Insert barcode image
        p.drawInlineImage(barcode_buffer, width - 250, height - 75, width=200, height=40)
        
    except Exception as e:
        print(f"Barcode generation failed: {e}")
        # Fallback: draw simple barcode-like lines
        p.setFillColor(colors.black)
        for i in range(20):
            x_pos = width - 200 + (i * 8)
            line_height = 30 if i % 3 == 0 else 25
            p.rect(x_pos, height - 65, 3, line_height, fill=1, stroke=0)
    
    # Order number text below barcode
    p.setFillColor(colors.white)
    p.setFont("Helvetica-Bold", 12)
    p.drawCentredText(width - 150, height - 85, f"NO PESANAN {order_number}")
    
    # Main content area
    y_pos = height - 120
    
    # Item section (left side)
    p.setFillColor(dark_gray)
    p.setFont("Helvetica-Bold", 16)
    p.drawString(left_margin, y_pos, "Jenis Barang")
    ...
    total_items = 0
    for item in order.order_items:
        if item_count < 3:  # Limit to 3 items for space
            item_name = item.product.name[:30] + "..." if len(item.product.name) > 30 else item.product.name
            p.drawString(left_margin + 10, y_pos, f"• {item_name}")
            y_pos -= 18
            total_items += item.quantity
            item_count += 1
    
    if len(order.order_items) > 3:
        p.drawString(left_margin + 10, y_pos, f"• +{len(order.order_items) - 3} item lainnya")
        y_pos -= 18
    
    # Add separator line
    p.setStrokeColor(dark_gray)
    p.setLineWidth(1)
    p.line(left_margin, y_pos - 10, width/2 - 20, y_pos - 10)
    
    # Price and weight section
    y_pos -= 30
    p.setFillColor(dark_gray)
    p.setFont("Helvetica-Bold", 16)
    p.drawString(left_margin, y_pos, "Harga")
    
    y_pos -= 25
    p.setFont("Helvetica-Bold", 18)
    p.drawString(left_margin, y_pos, order.formatted_total)
    
    y_pos -= 35
    p.setFont("Helvetica-Bold", 12)
    # Calculate total weight
    total_weight = sum(item.quantity * (item.product.weight or 0) for item in order.order_items) / 1000  # Convert to kg
    p.drawString(left_margin, y_pos, f"Berat {total_weight:.1f}kg")
    
    # Recipient section (center-right)
    p.setFillColor(dark_gray)
    p.setFont("Helvetica-Bold", 18)
    p.drawString(width/2, height - 140, "Penerima")
    
    y_pos = height - 165
    p.setFont("Helvetica-Bold", 14)
    p.drawString(width/2, y_pos, order.user.name)
    ...
    if order.user.address:
        ...
        for line in lines[:3]:  # Limit to 3 lines
            p.drawString(width/2, y_pos, line)
            y_pos -= 15
    
    if order.user.phone:
        y_pos -= 5
        p.setFont("Helvetica-Bold", 10)
        p.drawString(width/2, y_pos, f"Telp: {order.user.phone}")
    
    # Sender section (right side)
    sender_x = width - 180
    p.setFillColor(dark_gray)
    p.setFont("Helvetica-Bold", 18)
    p.drawString(sender_x, height - 140, "Pengirim")
    
    y_pos = height - 165
    p.setFont("Helvetica-Bold", 12)
    sender_name = store_profile.store_name if store_profile else "Hurtrock Music"
    p.drawString(sender_x, y_pos, sender_name)
    ...
    if store_profile:
        ...
        sender_address = store_profile.formatted_address
        sender_words = sender_address.split(' ')
        sender_lines = []
        current_line = ""
        max_chars = 20
        ...
        for word in sender_words:
            if len(current_line + word) <= max_chars:
                ...
                if current_line:
                    sender_lines.append(current_line.strip())
                current_line = word + " "
        if current_line:
            sender_lines.append(current_line.strip())
        
        for line in sender_lines[:3]:
            p.drawString(sender_x, y_pos, line)
            y_pos -= 12
            
        if store_profile.store_phone:
            y_pos -= 3
            p.setFont("Helvetica-Bold", 9)
            p.drawString(sender_x, y_pos, f"Telp: {store_profile.store_phone}")
    ...
    try:
        ...
        # Create QR code with order info
        ...
        # Draw QR code
        p.drawInlineImage(qr_buffer, left_margin, footer_y - 60, width=60, height=60)
        
    except Exception as e:
        print(f"QR code generation failed: {e}")
        # Fallback: draw placeholder box
        p.setFillColor(light_gray)
        p.rect(left_margin, footer_y - 60, 60, 60, fill=1, stroke=1)
    
    # Service info
    p.setFillColor(dark_gray)
    p.setFont("Helvetica-Bold", 10)
    p.drawString(left_margin + 80, footer_y - 10, f"Layanan: {order.courier_service or 'Reguler'}")
    p.setFont("Helvetica", 9)
    p.drawString(left_margin + 80, footer_y - 25, f"Tanggal: {order.created_at.strftime('%d/%m/%Y %H:%M')}")
    p.drawString(left_margin + 80, footer_y - 40, f"Estimasi: {order.estimated_delivery_days or 3} hari kerja")
    
    # Footer brand
    p.setFont("Helvetica", 8)
    website = store_profile.store_website if store_profile else "hurtrock.com"
    p.drawString(left_margin + 80, footer_y - 55, f"www.{website}")
    
    # Green accent stripe
    p.setFillColor(accent_color)
    p.polygon([(width - 100, height), (width, height), (width, height - 100)], fill=1, stroke=0)
    ...
    return send_file(
        buffer,
        as_attachment=True,
        download_name=f'professional_label_{order.id}.pdf',
        mimetype='application/pdf'
    )
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--server-mode':
        ...
        if is_production:
            ...
            # Development mode with reloader (default)
