...
def api_buyer_rooms():
    try:
        ...
        response = requests.get(chat_service_url, headers=headers)
        return jsonify(response.json()), response.status_code
        
    except Exception as e:
        print(f"Error proxying buyer rooms request: {str(e)}")
        return jsonify({'error': 'Failed to get buyer rooms'}), 500

@app.route('/api/rooms/<room_name>/messages/')
@login_required
def api_room_messages(room_name):
    try:
        import requests
        chat_service_url = f"http://localhost:8000/api/rooms/{room_name}/messages/"
        ...
        print(f"Error proxying room messages request: {str(e)}")
        return jsonify({'error': 'Failed to get room messages'}), 500

@app.route('/api/rooms/<room_name>/mark-read/', methods=['POST'])
@login_required
def api_mark_room_read(room_name):
    try:
        import requests
        ...
        response = requests.post(chat_service_url, headers=headers)
        return jsonify(response.json()), response.status_code
