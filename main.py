...
def admin_send_reply():
    try:
        print(f"=== Admin Send Reply Called ===")
        ...
        print(f"Parsed data - Room ID: '{room_id}', Message: '{message_text}', Product ID: '{product_id}'")
        
        # Validasi sama seperti floating chat
        if not message_text or message_text.strip() == '':
            return jsonify({'error': 'Message cannot be empty'}), 400
        ...
        # Process product_id
        processed_product_id = None
        if product_id and product_id.strip():
            try:
                processed_product_id = int(product_id)
            except (ValueError, TypeError):
                processed_product_id = None
        
        # Create message - sama struktur seperti floating chat
        message = models.ChatMessage(
            chat_room_id=room_id,
            sender_type='admin',
            message=message_text.strip(),
            product_id=processed_product_id
        )
        db.session.add(message)
        ...
        # Emit to user - sama seperti floating chat mengirim ke admin
        emit_data = {
            'message': message_text.strip(),
            'sender_type': 'admin',
            'timestamp': message.created_at.strftime('%H:%M'),
        ...
        socketio.emit('new_message', emit_data, room=f'user_{chat_room.user_id}')
        print(f"Emitted to user_{chat_room.user_id}: {emit_data}")
...
def on_join(data):
    if current_user.is_authenticated:
        if current_user.is_admin or current_user.is_staff:
            ...
            print(f"Admin/Staff {current_user.name} joined admin_room")
        else:
            user_room = f'user_{current_user.id}'
            join_room(user_room)
            print(f"User {current_user.name} joined {user_room}")
...
def on_disconnect():
    if current_user.is_authenticated:
        if current_user.is_admin or current_user.is_staff:
            ...
            print(f"Admin/Staff {current_user.name} left admin_room")
        else:
            user_room = f'user_{current_user.id}'
            leave_room(user_room)
            print(f"User {current_user.name} left {user_room}")

@socketio.on('connect')
def on_connect():
    if current_user.is_authenticated:
        print(f"User {current_user.name} (ID: {current_user.id}) connected via Socket.IO")
    else:
