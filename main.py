...
if not configure_database(app):
    print("❌ Gagal mengkonfigurasi database, aplikasi akan berhenti")
    exit(1)
...
JWT_SECRET_KEY = os.environ.get("SESSION_SECRET")  # Use same secret as session
JWT_ALGORITHM = 'HS256'
...
def setup_django_chat_service():
    ...
    chat_service_dir = os.path.join(os.path.dirname(__file__), 'chat_service')
    
    if not os.path.exists(chat_service_dir):
        print("❌ Chat service directory not found")
        return False
    ...
    try:
        ...
        os.chdir(chat_service_dir)
        
        # Run Django migrations
        print("🔧 Running Django chat service migrations...")
        
        # Make migrations
        subprocess.run([sys.executable, 'manage.py', 'makemigrations', 'chat'], 
        ...
        print("✅ Django chat service migrations completed")
        ...
        print(f"❌ Django migration error: {e}")
        os.chdir(original_cwd)
        return False
...
with app.app_context():
    try:
        ...
        if setup_django_chat_service():
            print("✅ Django chat service configured")
        else:
            print("⚠️  Django chat service setup failed, continuing...")
...
def api_product_detail(product_id):
    """API endpoint to get product details as JSON for chat service"""
    product = models.Product.query.get_or_404(product_id)
    return jsonify({
        'id': product.id,
        'name': product.name,
...
def proxy_buyer_rooms():
    try:
        search_query = request.args.get('search', '')
        chat_service_url = f"http://localhost:8000/api/admin/buyer-rooms/"
        if search_query:
            chat_service_url += f"?search={search_query}"
        ...
        response = requests.get(chat_service_url, headers=headers, timeout=10)
        
        if response.status_code == 200:
            return jsonify(response.json()), response.status_code
        else:
            print(f"Chat service returned status {response.status_code}: {response.text}")
            return jsonify({'error': 'Chat service error', 'rooms': [], 'total_count': 0}), response.status_code

    except requests.exceptions.RequestException as e:
        print(f"Error proxying buyer rooms request: {str(e)}")
        return jsonify({'error': 'Chat service unavailable', 'rooms': [], 'total_count': 0}), 503
    except ValueError as e:
        print(f"JSON decode error in proxy_buyer_rooms: {str(e)}")
        return jsonify({'error': 'Invalid response from chat service', 'rooms': [], 'total_count': 0}), 502
    except Exception as e:
...
def proxy_room_messages(room_name):
    try:
        chat_service_url = f"http://localhost:8000/api/rooms/{room_name}/messages/"

        # Forward query parameters
        query_params = request.args.to_dict()
        if query_params:
            query_string = "&".join([f"{k}={v}" for k, v in query_params.items()])
            chat_service_url += f"?{query_string}"
        ...
        if response.status_code == 200:
            try:
                ...
                print(f"JSON decode error in proxy_room_messages: {str(e)}")
                print(f"Response content: {response.text}")
                return jsonify({'error': 'Invalid response from chat service', 'results': []}), 502
        else:
            print(f"Chat service returned status {response.status_code}: {response.text}")
            return jsonify({'error': 'Chat service error', 'results': []}), response.status_code

    except requests.exceptions.RequestException as e:
        print(f"Error proxying room messages request: {str(e)}")
...
def proxy_mark_room_read(room_name):
    try:
        chat_service_url = f"http://localhost:8000/api/rooms/{room_name}/mark-read/"
        ...
        response = requests.post(chat_service_url, headers=headers, timeout=10)
        ...
        if response.status_code == 200:
            ...
            print(f"Chat service returned status {response.status_code}: {response.text}")
            return jsonify({'error': 'Failed to mark messages as read'}), response.status_code
        
    except requests.exceptions.RequestException as e:
        print(f"Error proxying mark read request: {str(e)}")
