...
def admin_create_payment_config():
    if request.method == 'POST':
        ...
        if provider == 'midtrans':
            config.midtrans_client_key = request.form.get('midtrans_client_key')
            config.midtrans_server_key = request.form.get('midtrans_server_key')
            config.midtrans_merchant_id = request.form.get('midtrans_merchant_id')
        ...
        config.notification_url = f"{base_url}/payment/notification"
        
        db.session.add(config)
        db.session.commit()
        
        flash(f'Konfigurasi pembayaran {provider} berhasil ditambahkan!', 'success')
